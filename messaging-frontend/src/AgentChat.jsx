...
function AgentChat({ conversationId, agentId, agents = [], customers = [] }) {
  ...
  const [customerSelectedFile, setCustomerSelectedFile] = useState(null);
  ...
  useEffect(() => {
    // Listen for file selection from CustomerChat
    const updateCustomerFile = () => {
      const fileInfo = window.localStorage.getItem('customerSelectedFile');
      setCustomerSelectedFile(fileInfo ? JSON.parse(fileInfo) : null);
    };
    window.addEventListener('customerSelectedFileChanged', updateCustomerFile);
    updateCustomerFile();
    return () => {
      if (stompClient.current) {
        stompClient.current.deactivate();
    ...
    <div className="chat-container">
      <div className="messages-area">
        {messages.map((msg, index) => {
          ...
          const isSent = agents.some(a => a.id === msg.senderId);
      ...
      <div className="input-area">
        ...
        {customerSelectedFile && (
          <div className="file-preview" style={{ display: 'flex', alignItems: 'center', marginBottom: 6 }}>
            {customerSelectedFile.type && customerSelectedFile.type.startsWith('image') ? (
              <span style={{ color: '#10b981', marginRight: 8 }}>🖼️</span>
            ) : null}
            <span style={{ color: '#10b981', marginRight: 8, maxWidth: 120, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', display: 'inline-block' }}>{customerSelectedFile.name}</span>
          </div>
        )}
        {selectedFile && (
          <div className="file-preview" style={{ display: 'flex', alignItems: 'center', marginBottom: 6 }}>
        ...
        <input
          ...
          style={editingMsgId ? { border: '2px solid #f59e42' } : {}}
          id="agent-chat-input"
        />
